...
class ResultReporter {
    static DASHBOARD_FILE = path_1.default.join(__dirname, '../../../web-applications/bob/dashboard.md');
    /**
     * Generate execution report
     */
    static generateReport(stepId, phaseId, action, commands, results) {
        const overallSuccess = results.every(r => r.success);
        const totalDuration = results.reduce((sum, r) => sum + r.duration, 0);
        const errors = results
            .filter(r => !r.success)
            .map(r => `${r.command}: ${r.stderr}`);
        const fixesApplied = []; // Would be populated by error handler
        return {
            stepId,
            phaseId,
            action,
     ...
     */
    static async updateFrameworkStatus(report) {
        try {
    ...
    static async advanceWorkflow() {
        try {
            ...
            const result = await bob_engine_1.BobEngine.run({ auto: false });
            if (result.success) {
     ...
     */
    static generateUserSummary(report) {
