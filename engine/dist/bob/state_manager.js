...
exports.BobStateManager = void 0;
const fs_extra_1 = __importDefault(require("fs-extra"));
const path_1 = __importDefault(require("path"));
const config_1 = require("../shared/config");
const config = config_1.BobConfig.getInstance();
const ROOT_DIR = config.getRootDir();
const FRAMEWORK_DIR = path_1.default.join(ROOT_DIR, config.getDirectory('framework'));
const PROJECT_DIR = path_1.default.join(ROOT_DIR, config.getDirectory('dashboard'));
const STATUS_FILE = path_1.default.join(PROJECT_DIR, 'framework_status.json');
const PHASES_FILE = path_1.default.join(FRAMEWORK_DIR, 'phases_definition.json');
class BobStateManager {
    ...
    static async loadStatus() {
        ...
        if (!await fs_extra_1.default.pathExists(STATUS_FILE)) {
            const templatePath = path_1.default.join(FRAMEWORK_DIR, 'framework_status.json');
            const schemaPath = path_1.default.join(FRAMEWORK_DIR, 'framework_status.schema.json');
            const targetSchemaPath = path_1.default.join(PROJECT_DIR, 'framework_status.schema.json');
            if (await fs_extra_1.default.pathExists(templatePath)) {
                // Ensure project directory exists
                await fs_extra_1.default.ensureDir(PROJECT_DIR);
                // Copy template to project directory
                await fs_extra_1.default.copy(templatePath, STATUS_FILE);
                // Copy schema to project directory if it exists
                if (await fs_extra_1.default.pathExists(schemaPath)) {
                    await fs_extra_1.default.copy(schemaPath, targetSchemaPath);
                }
            }
            else {
        ...
        const data = await fs_extra_1.default.readFile(STATUS_FILE, 'utf8');
    ...
    static async saveStatus(status) {
        ...
        await fs_extra_1.default.ensureDir(PROJECT_DIR);
        await fs_extra_1.default.writeFile(STATUS_FILE, JSON.stringify(status, null, 2), 'utf8');
    }
    /**
     * Load phase definitions
     */
    static async loadPhaseDefinitions() {
        if (!await fs_extra_1.default.pathExists(PHASES_FILE)) {
            throw new Error(`Phase definitions not found at ${PHASES_FILE}`);
        }
        const data = await fs_extra_1.default.readFile(PHASES_FILE, 'utf8');
     ...
     */
    static async updateStepStatus(phaseId, stepId, stepStatus, result, error) {
        const status = await this.loadStatus();
     ...
     * Update phase status
     */
    static async updatePhaseStatus(phaseId, phaseStatus) {
        const status = await this.loadStatus();
        if (!status.phases[phaseId]) {
            throw new Error(`Phase ${phaseId} not found in status`);
        }
     ...
     */
    static async advanceState(forceNextPhase = false) {
        const status = await this.loadStatus();
     ...
     */
    static async rollbackPreviousStep() {
        const status = await this.loadStatus();
        const phases = await this.loadPhaseDefinitions();
        const currentPhaseIndex = phases.phases.findIndex(p => p.id === status.current_phase);
     ...
     * Record an error
     */
    static async recordError(phase, step, message) {
        const status = await this.loadStatus();
        const error = {
            phase,
            step,
     ...
     */
    static async getPhaseAndStep(phaseId, stepId) {
        const phases = await this.loadPhaseDefinitions();
     ...
     */
    static async shouldSkipPhase(phaseId) {
        const status = await this.loadStatus();
    ...
    static getLayerForPhase(phaseId) {
    ...
    static async reset() {
        const phases = await this.loadPhaseDefinitions();
        const existingStatus = await this.loadStatus().catch(() => null);
...
exports.BobStateManager = BobStateManager;
