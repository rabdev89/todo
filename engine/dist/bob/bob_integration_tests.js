...
exports.BOB_ACTION_TEST_CASES = void 0;
exports.runBobIntegrationSmoke = runBobIntegrationSmoke;
exports.runBobIntegrationAuto = runBobIntegrationAuto;
exports.printBobActionTestCases = printBobActionTestCases;
exports.runBobForAction = runBobForAction;
exports.runValidateRepositoryStructure = runValidateRepositoryStructure;
exports.runDocImplementationAlignmentForCase = runDocImplementationAlignmentForCase;
exports.runDocImplementationAlignmentAll = runDocImplementationAlignmentAll;
const bob_engine_1 = require("./bob_engine");
const phase_router_1 = require("./phase_router");
const state_manager_1 = require("./state_manager");
const framework_1 = require("./action_handlers/framework");
const project_1 = require("./action_handlers/project");
const architecture_1 = require("./action_handlers/architecture");
const planning_1 = require("./action_handlers/planning");
const development_1 = require("./action_handlers/development");
const fs_extra_1 = __importDefault(require("fs-extra"));
const path_1 = __importDefault(require("path"));
exports.BOB_ACTION_TEST_CASES = [
    {
...
async function runBobIntegrationSmoke() {
    await bob_engine_1.BobEngine.run({ mode: 'full' });
}
async function runBobIntegrationAuto() {
    await bob_engine_1.BobEngine.run({ auto: true, mode: 'full' });
}
function printBobActionTestCases() {
    for (const test of exports.BOB_ACTION_TEST_CASES) {
        console.log('------------------------------------------------------------');
...
async function runBobForAction(action) {
    ...
    await phase_router_1.BobPhaseRouter.jumpTo(targetPhaseId, targetStepId);
    return bob_engine_1.BobEngine.run({ mode: 'full' });
}
async function runValidateRepositoryStructure() {
    return runBobForAction('validate_repository_structure');
}
function includesAllSignals(logs, must) {
...
async function runDocImplementationAlignmentForCase(test) {
    switch (test.phase) {
...
async function validateFrameworkBootstrapCase(test) {
    let lastResultLogs;
    if (test.action === 'validate_repository_structure') {
        const initial = await (0, framework_1.validateRepositoryStructure)({});
        if (!initial.success) {
        ...
        for (const exp of test.expectations || []) {
            if (exp.kind === 'repository-structure') {
                const dirs = initial.data?.dirs || [];
                const files = initial.data?.files || [];
    ...
    for (const exp of test.expectations || []) {
        if (exp.kind === 'framework-version' && test.action === 'validate_framework_version') {
            const result = await (0, framework_1.validateFrameworkVersion)({});
            if (!result.success) {
            ...
            const result = await (0, framework_1.runFrameworkDiagnostics)({});
            ...
            let installationType = 'normal';
            try {
                const fwStatusPath = path_1.default.resolve(ROOT_DIR, 'framework', 'framework_status.json');
                if (await fs_extra_1.default.pathExists(fwStatusPath)) {
                    const fw = await fs_extra_1.default.readJson(fwStatusPath);
                    if (fw.installation_type === 'complete')
                        installationType = 'complete';
                }
            }
            catch { }
            for (const file of exp.files) {
                const p = path_1.default.isAbsolute(file) ? file : path_1.default.resolve(ROOT_DIR, file);
                if (!await fs_extra_1.default.pathExists(p)) {
                    throw new Error(`health file not found: ${p}`);
                }
                const content = await fs_extra_1.default.readJson(p);
...
async function validateDevelopmentAndOpsCase(test) {
    for (const exp of test.expectations || []) {
        if (test.action === 'select_next_ticket') {
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.selectNextTicket)({});
                if (!result.success) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.generateCode)({});
                if (!result.success) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.reviewCode)({});
                if (!result.success) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.generateUnitTests)({});
                if (!result.success) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.runTests)({});
                if (!result.success) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.runIntegrationTests)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.fixBugs)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.updateDocumentation)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.validateEpic)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.runSystemTests)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.runPerformanceTests)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.runSecurityAudit)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.buildReleaseCandidate)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.setupUatEnvironment)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.runUat)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.collectUatFeedback)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.fixUatBugs)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.generateReleaseNotes)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.generateDeploymentScripts)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.generateRollbackPlan)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.generateDeploymentChecklist)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.validatePreLaunch)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.deployToProduction)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.verifyDeployment)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, development_1.monitorPerformance)({});
                if (!includesAllSignals(result.logs, exp.mustContain)) {
...
async function validateProductDefinitionCase(test) {
    const ROOT_DIR = path_1.default.resolve(process.cwd(), '..');
    for (const exp of test.expectations || []) {
        if (test.action === 'generate_vision_document') {
            if (exp.kind === 'artifacts') {
                const result = await (0, project_1.generateVisionDocument)({});
                if (!result.success) {
                    throw new Error(result.error || 'generate vision document failed');
                }
                for (const pth of exp.paths) {
                    const full = path_1.default.isAbsolute(pth) ? pth : path_1.default.resolve(ROOT_DIR, pth);
            ...
            if (exp.kind === 'artifacts') {
                const result = await (0, project_1.generateUserFlow)({});
                if (!result.success) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, project_1.validateUserFlow)({});
                if (!result.success) {
            ...
            if (exp.kind === 'artifacts') {
                const result = await (0, project_1.generateRequirements)({});
                if (!result.success) {
...
async function validateTechnicalArchitectureCase(test) {
    const ROOT_DIR = path_1.default.resolve(process.cwd(), '..');
    for (const exp of test.expectations || []) {
        if (test.action === 'generate_architecture_design') {
            if (exp.kind === 'artifacts') {
                const result = await (0, architecture_1.generateArchitectureDesign)({});
                if (!result.success) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, architecture_1.validateTechStack)({});
                if (!result.success) {
            ...
            if (exp.kind === 'artifacts') {
                const result = await (0, architecture_1.generateDatabaseSchema)({});
                if (!result.success) {
            ...
            if (exp.kind === 'artifacts') {
                const result = await (0, architecture_1.generateApiContracts)({});
                if (!result.success) {
...
async function validateProjectPlanningCase(test) {
    const ROOT_DIR = path_1.default.resolve(process.cwd(), '..');
    for (const exp of test.expectations || []) {
        if (test.action === 'generate_epics') {
            if (exp.kind === 'artifacts') {
                const result = await (0, planning_1.generateEpics)({});
                if (!result.success) {
            ...
            if (exp.kind === 'artifacts') {
                const result = await (0, planning_1.generateTickets)({});
                if (!result.success) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, planning_1.validateTickets)({});
                if (!result.success) {
            ...
            if (exp.kind === 'artifacts') {
                const result = await (0, planning_1.generateProjectTimeline)({});
                if (!result.success) {
...
async function validateFrameworkInstallationCase(test) {
    const ROOT_DIR = path_1.default.resolve(process.cwd(), '..');
    for (const exp of test.expectations || []) {
        if (test.action === 'install_framework_dependencies') {
            if (exp.kind === 'log-signals') {
                const result = await (0, framework_1.installFrameworkDependencies)({});
                if (!result.success) {
                ...
                for (const p of exp.paths) {
                    const full = path_1.default.isAbsolute(p) ? p : path_1.default.resolve(ROOT_DIR, p);
                    if (!await fs_extra_1.default.pathExists(full)) {
                        throw new Error(`artifact not found: ${full}`);
                    }
                }
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, framework_1.validateEnvironment)({});
                if (!result.success) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, framework_1.runFrameworkTests)({});
                if (!result.success) {
            ...
            if (exp.kind === 'health-files') {
                for (const file of exp.files) {
                    const full = path_1.default.isAbsolute(file) ? file : path_1.default.resolve(ROOT_DIR, file);
                    if (!await fs_extra_1.default.pathExists(full)) {
                        throw new Error(`health report not found: ${full}`);
                    }
                    const content = await fs_extra_1.default.readJson(full);
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, framework_1.startEngineServices)({});
                if (!result.success) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, framework_1.startWatchdogService)({});
                if (!result.success) {
...
async function validateProjectInitializationCase(test) {
    const ROOT_DIR = path_1.default.resolve(process.cwd(), '..');
    for (const exp of test.expectations || []) {
        if (test.action === 'start_project_init') {
            if (exp.kind === 'log-signals') {
                const result = await (0, project_1.startProjectInit)({});
                if (!result.success) {
            ...
            if (exp.kind === 'log-signals') {
                const result = await (0, project_1.validateFrameworkProjectAlignment)({});
                if (!result.success) {
            ...
            if (exp.kind === 'log-signals') {
                ...
                const result = await (0, project_1.promptProjectType)(mockContext);
                if (!result.success) {
            ...
            if (exp.kind === 'artifacts') {
                const result = await (0, project_1.generateTechStack)({});
                if (!result.success) {
            ...
            if (exp.kind === 'artifacts') {
                const result = await (0, project_1.generateProjectContext)({});
                if (!result.success) {
            ...
            if (exp.kind === 'artifacts') {
                const result = await (0, project_1.generateProjectManagementStructure)({});
                if (!result.success) {
...
async function runGenericExpectations(test) {
    for (const exp of test.expectations || []) {
        if (exp.kind === 'log-signals') {
...
async function runDocImplementationAlignmentAll() {
    ...
    const cases = exports.BOB_ACTION_TEST_CASES.filter(c => phasesToRun.has(c.phase));
    let passed = 0;
    let failed = 0;
    const failures = [];
