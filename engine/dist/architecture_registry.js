...
exports.ArchitectureRegistry = void 0;
...
class ArchitectureRegistry {
     ...
     */
    registerModule(name, type, primaryFile, options = {}) {
    ...
    query(query) {
        return Object.values(this.data.modules).filter(module => {
            if (query.type && module.type !== query.type)
                return false;
            if (query.name && !module.name.toLowerCase().includes(query.name.toLowerCase()))
                return false;
            if (query.filePath) {
            ...
            if (query.responsibility) {
                const hasResponsibility = module.responsibilities.some(r => r.toLowerCase().includes(query.responsibility.toLowerCase()));
     ...
     */
    suggestExistingModule(desiredResponsibility, desiredType) {
     ...
     */
    suggestModuleForResponsibility(responsibility) {
     ...
     */
    autoDetectModules(sourcePath = './src', ticketId) {
        ...
        for (const [type, pattern] of Object.entries(patterns)) {
            ...
            const files = (0, glob_1.globSync)(globPattern, { cwd: process.cwd() });
            for (const file of files) {
                ...
                const module = this.registerModule(name, type, file, {
     ...
     */
    buildContextForTicket(ticketModule) {
...
exports.ArchitectureRegistry = ArchitectureRegistry;
exports.default = ArchitectureRegistry;
