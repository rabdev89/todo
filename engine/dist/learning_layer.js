...
class LearningLayer {
    ...
    metricsPath;
    signalsPath;
    constructor(dataDir = '.engine/learning') {
     ...
     */
    async recordTicketExecution(record) {
    ...
    calculateMetrics(history) {
        ...
        const phaseFailureRates = new Map();
    ...
    detectSignals(history) {
        ...
        const complexTickets = records.filter(r => r.filesModified.length > 5 || r.attempts > 2);
     ...
     */
    getTicketRecommendation(ticketId) {
        const history = this.loadHistory();
        const metrics = this.calculateMetrics(history);
        ...
        if (!ticket)
            return null;
        const similar = history.filter(h => h.ticketId !== ticketId &&
...
exports.LearningLayer = LearningLayer;
