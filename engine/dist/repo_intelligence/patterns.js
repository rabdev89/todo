...
class PatternDetector {
    ...
    detectAPIFramework() {
        ...
        // Check for FastAPI
        const fastapiChunks = chunks.filter(c => c.content.includes('from fastapi import') ||
        ...
        // Check for Express
        const expressChunks = chunks.filter(c => c.content.includes("require('express')") ||
    ...
    detectORMPattern() {
        ...
        const sqlalchemyChunks = chunks.filter(c => c.content.includes('from sqlalchemy') ||
        ...
        const djangoChunks = chunks.filter(c => c.content.includes('from django.db import models') ||
    ...
    detectTestFramework() {
        const chunks = this.storage.getAllChunks();
        // Check for pytest
        const pytestChunks = chunks.filter(c => c.content.includes('import pytest') ||
        ...
        // Check for Jest
        const jestChunks = chunks.filter(c => c.content.includes('import') && c.content.includes('from \'@jest/globals\'') ||
    ...
    extractSymbolsFromChunk(chunk) {
        // This is a simplified implementation
        // In practice, you'd re-parse the chunk content
        return [];
    }
    hasDependencyInjection(symbol) {
    ...
    detectStrategyPattern() {
        const strategyClasses = this.storage.getSymbolsByName('')
            .filter(s => (s.name.toLowerCase().includes('strategy') ||
    ...
    detectMiddlewarePattern() {
        const chunks = this.storage.getAllChunks();
        const middlewareChunks = chunks.filter(c => c.content.toLowerCase().includes('middleware') ||
            c.content.toLowerCase().includes('interceptor') ||
            c.content.toLowerCase().includes('@app.middleware'));
        if (middlewareChunks.length < 3)
            return null;
        return {
            name: 'Middleware',
    ...
    detectErrorHandlingPattern() {
        const chunks = this.storage.getAllChunks();
        const errorChunks = chunks.filter(c => c.content.toLowerCase().includes('try:') ||
            c.content.toLowerCase().includes('except') ||
            c.content.toLowerCase().includes('catch') ||
            c.content.toLowerCase().includes('error'));
        if (errorChunks.length < 10)
            return null;
        return {
            name: 'ErrorHandling',
    ...
    detectConfigurationPattern() {
        const configClasses = this.storage.getSymbolsByName('')
            .filter(s => (s.name.toLowerCase().includes('config') ||
    ...
    detectLoggingPattern() {
        const chunks = this.storage.getAllChunks();
        const logChunks = chunks.filter(c => c.content.toLowerCase().includes('log') ||
    ...
    detectCachingPattern() {
        const chunks = this.storage.getAllChunks();
        const cacheChunks = chunks.filter(c => c.content.toLowerCase().includes('cache') ||
            c.content.toLowerCase().includes('redis') ||
            c.content.toLowerCase().includes('memoize'));
        if (cacheChunks.length < 3)
            return null;
        return {
            name: 'Caching',
    ...
    detectValidationPattern() {
        const validationClasses = this.storage.getSymbolsByName('')
            .filter(s => (s.name.toLowerCase().includes('validator') ||
            s.name.toLowerCase().includes('validation') ||
            s.name.toLowerCase().includes('schema')) &&
            s.type === 'class');
        if (validationClasses.length < 2)
            return null;
        return {
            name: 'Validation',
