...
class ContextBuilder {
    ...
    patterns;
    storage;
    constructor(searchService, patternDetector, storage) {
        this.search = searchService;
        this.patterns = patternDetector;
        this.storage = storage;
    }
    async buildContext(ticket) {
    ...
    extractKeywordsFromTicket(ticket) {
    ...
    suggestSkills(ticket, patterns, searchResults) {
        const skills = [];
        const text = `${ticket.title} ${ticket.description}`.toLowerCase();
    ...
    calculateConfidence(searchResults, symbolResults, patterns) {
        let confidence = 0;
    ...
    generateRationale(ticket, searchResults, patterns) {
        const parts = [];
