...
export class SecurityGuard implements IValidationGuard {
    ...
    async run(ticketId: string): Promise<GuardResult> {
        ...
        const hasLockFile = await fs.pathExists(path.join(rootDir, 'package-lock.json')) || 
                           await fs.pathExists(path.join(rootDir, 'yarn.lock')) ||
                           await fs.pathExists(path.join(rootDir, 'pnpm-lock.yaml'));
