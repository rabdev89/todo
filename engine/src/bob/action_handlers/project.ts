...
const WEB_APP_DIR = path.join(ROOT_DIR, 'web-applications/bob');
const WEB_APP_PM_DIR = path.join(ROOT_DIR, 'web-applications/project-management');
const WEB_APP_DESIGN_DIR = path.join(WEB_APP_PM_DIR, 'design');
const FRAMEWORK_DIR = path.join(ROOT_DIR, 'framework');

/**
 * Start project initialization
 */
export const startProjectInit: ActionHandler = async (
  context
 ...
 */
export const selectProjectType: ActionHandler = async (
  context
): Promise<ActionResult> => {
 ...
 */
export const promptProjectType: ActionHandler = async (
  context
): Promise<ActionResult> => {
  console.log('\n📋 Project Type Selection\n');

  // Check for mock data in context (for testing)
  const mockProjectType = (context as any).mockProjectType;
  const mockProjectName = (context as any).mockProjectName;
 ...
 */
export const validateFrameworkProjectAlignment: ActionHandler = async (
  context
): Promise<ActionResult> => {
 ...
 */
export const generateTechStack: ActionHandler = async (
  context
): Promise<ActionResult> => {
  ...
  if (status.project_type === 'continue_project') {
    ...
    const webAppDir = path.join(ROOT_DIR, 'web-applications');
    const detectedTechStack: any = {
      detected: true,
    ...
    try {
      const entries = await fs.readdir(webAppDir);
      for (const entry of entries) {
        if (entry === 'bob') continue; // Skip bob management folder
        if (entry === 'project-management') continue; // Skip bob management folder
        
        const full = path.join(webAppDir, entry);
        try {
          const stat = await fs.stat(full);
          if (!stat.isDirectory()) continue;

          const projectInfo: any = { name: entry };

          // Detect common files
          const pkgPath = path.join(full, 'package.json');
          const reqPath = path.join(full, 'requirements.txt');
          const pyProj = path.join(full, 'pyproject.toml');
          const pubspecPath = path.join(full, 'pubspec.yaml');
          const pomPath = path.join(full, 'pom.xml');
          const gradlePath = path.join(full, 'build.gradle');
          const csprojFiles = (await fs.readdir(full)).filter(f => f.endsWith('.csproj'));
          const dockerfile = path.join(full, 'Dockerfile');
          ...
          // Python backends
          if (await fs.pathExists(reqPath) || await fs.pathExists(pyProj)) {
            projectInfo.role = 'backend';
            // try to detect framework from requirements/pyproject
            try {
              const reqContent = await fs.readFile(await fs.pathExists(pyProj) ? pyProj : reqPath, 'utf8');
              if (/fastapi/i.test(reqContent)) projectInfo.framework = 'FastAPI';
              else if (/django/i.test(reqContent)) projectInfo.framework = 'Django';
              else if (/flask/i.test(reqContent)) projectInfo.framework = 'Flask';
              else projectInfo.framework = projectInfo.framework || 'Python (unspecified)';
            } catch (e) {
              projectInfo.framework = projectInfo.framework || 'Python (unspecified)';
            }
            projectInfo.language = 'Python';
            projectInfo.package_manager = await fs.pathExists(pyProj) ? 'poetry' : 'pip';
          }
          ...
          if (await fs.pathExists(pomPath) || await fs.pathExists(gradlePath)) {
            ...
            projectInfo.package_manager = await fs.pathExists(pomPath) ? 'maven' : 'gradle';
          }
          ...
          // Docker / compose
          if (await fs.pathExists(dockerfile) || await fs.pathExists(compose)) {
    ...
    const techStackPath = path.join(ROOT_DIR, 'web-applications', 'tech_stack.json');
    await fs.writeJson(techStackPath, detectedTechStack, { spaces: 2 });
  ...
  const techStackPath = path.join(WEB_APP_DIR, 'tech_stack.json');

  // If tech_stack.json already exists, skip AI prompt and proceed
  if (await fs.pathExists(techStackPath)) {
    const techStack = await fs.readJson(techStackPath);
    return {
      success: true,
 ...
 */
export const generateProjectContext: ActionHandler = async (
  context
): Promise<ActionResult> => {
  console.log('  Generating project context...');
  await BobLogger.info('generateProjectContext invoked');
  ...
  if (status.project_type === 'continue_project') {
    ...
    const analysis: any = {
      ...
      frontend_dirs: [] as string[],
      backend_dirs: [] as string[],
      config_files: [] as string[]
    };

    try {
      const entries: string[] = await fs.readdir(webAppDir);
      for (const entry of entries) {
        const fullPath = path.join(webAppDir, entry);
        try {
          const stat = await fs.stat(fullPath);
          if (!stat.isDirectory()) {
            const ext = path.extname(entry);
            const configExts = ['.json', '.yml', '.yaml'];
            if (configExts.includes(ext)) analysis.config_files.push(entry);
            continue;
          }
          ...
          if (await fs.pathExists(pkgPath)) {
            ...
            if (deps['@nestjs/core'] || deps['@nestjs/common'] || deps['express'] || deps['koa'] || deps.hapi) {
              ...
              if (!analysis.backend_dirs.includes(entry)) analysis.backend_dirs.push(entry);
            }
          }
          ...
          if (await fs.pathExists(pubspecPath)) {
            try {
              ...
              if (/flutter:/i.test(pubspec) || /sdk: flutter/i.test(pubspec)) {
                analysis.has_frontend = true;
                if (!analysis.frontend_dirs.includes(entry)) analysis.frontend_dirs.push(entry);
              }
            } catch (e) {
              // ignore
            }
          ...
          const pyProj = path.join(fullPath, 'pyproject.toml');
          if (await fs.pathExists(reqPath) || await fs.pathExists(pyProj)) {
            analysis.has_backend = true;
            if (!analysis.backend_dirs.includes(entry)) analysis.backend_dirs.push(entry);
          }
      ...
      await BobLogger.warn('Error analyzing project structure', { error: (error as Error).message });
    }
    ...
    const contextPath = path.join(WEB_APP_PM_DIR, 'project_context.json');
    await fs.ensureDir(WEB_APP_PM_DIR);
    await fs.writeJson(contextPath, existingContext, { spaces: 2 });
    await BobLogger.info('Project context analyzed and written', { contextPath });
  ...
  const contextPath = path.join(WEB_APP_PM_DIR, 'project_context.json');
  await fs.ensureDir(WEB_APP_PM_DIR);
  await fs.writeJson(contextPath, projectContext, { spaces: 2 });
  await BobLogger.info('Project context written', { contextPath });
 ...
 */
export const setupContinueProjectUat: ActionHandler = async (
  context
): Promise<ActionResult> => {
  console.log('  Setting up UAT for continue project...');
  await BobLogger.info('setupContinueProjectUat invoked');
 ...
 */
export const generateProjectManagementStructure: ActionHandler = async (
  context
): Promise<ActionResult> => {
  console.log('  Generating project management structure...');
  await BobLogger.info('generateProjectManagementStructure invoked');
  ...
  if (createdDirs.length > 0) {
    await BobLogger.info('Created project management directories', { createdDirs });
  }
...
`
  ...
  for (const template of templates) {
    ...
    if (!await fs.pathExists(filePath)) {
      await fs.outputFile(filePath, template.content, 'utf8');
      createdFiles.push(template.path);
    }
  }
  if (createdFiles.length > 0) {
    await BobLogger.info('Created project management template files', { createdFiles });
  }
 ...
 * Bob stays on this action until data provided, even if /bob called multiple times.
 */
export const generateVisionDocument: ActionHandler = async (
  context
): Promise<ActionResult> => {
  ...
  const visionQuestionnairePath = path.join(WEB_APP_PM_DIR, 'vision_questionnaire.md');
  ...
  await fs.writeFile(visionQuestionnairePath, visionQuestionnaireContent, 'utf8');  
  
  // Check if vision_details.json exists
  const visionDetailsPath = path.join(WEB_APP_PM_DIR, 'vision_details.json');
  const visionDetailsExists = await fs.pathExists(visionDetailsPath);

  // ===== SUCCESS PATH: Vision details file exists =====
  if (visionDetailsExists) {
    const visionDetails = await fs.readJson(visionDetailsPath);
    ...
    const visionContent = `# Vision: ${projectName}
      ...
      ${Array.isArray(visionDetails.key_features) 
        ? visionDetails.key_features.map((f: string) => `- ${f}`).join('\n')
      ...
      ${Array.isArray(visionDetails.success_metrics)
        ? visionDetails.success_metrics.map((m: string) => `- ${m}`).join('\n')
    ...
    const visionPath = path.join(WEB_APP_PM_DIR, 'vision.md');
    await fs.writeFile(visionPath, visionContent, 'utf8');
 ...
 */
export const userReviewVision: ActionHandler = async (
  context
): Promise<ActionResult> => {
  console.log('  Reviewing vision document...');

  const visionPath = path.join(WEB_APP_PM_DIR, 'vision.md');
  const approvalPath = path.join(WEB_APP_PM_DIR, 'vision_approval.json');
  ...
  if (!await fs.pathExists(approvalPath)) {
    const approval = await fs.writeJson(approvalPath, {
      approved: false,
      approved_by: null
    });
 ...
 * 7. Bob advances to next step
 */
export const generateUserFlow: ActionHandler = async (
  context
): Promise<ActionResult> => {
  ...
  const userFlowDraftPath = path.join(WEB_APP_PM_DIR, 'user_flow_draft.md');
  const userFlowDetailsPath = path.join(WEB_APP_PM_DIR, 'user_flow_details.json');
  const userFlowPath = path.join(WEB_APP_PM_DIR, 'user_flow.md');
...
*Date: ${new Date().toISOString()}*`;
  ...
  await fs.writeFile(userFlowDraftPath, userFlowDraftContent, 'utf8');  

  // Check if user_flow_details.json exists
  const userFlowDetailsExists = await fs.pathExists(userFlowDetailsPath);

  // ===== SUCCESS PATH: User flow details file exists =====
  if (userFlowDetailsExists) {
    const userFlowDetails = await fs.readJson(userFlowDetailsPath);
...
${Array.isArray(userFlowDetails.user_stories)
  ? userFlowDetails.user_stories.map((story: any) => 
...
${Array.isArray(userFlowDetails.screen_flow)
  ? userFlowDetails.screen_flow.map((screen: any, index: number) => 
      `${index + 1}. **${screen.name}**: ${screen.description}`
    ).join('\n')
...
`;
    ...
    await fs.writeFile(userFlowPath, userFlowContent, 'utf8');
 ...
 */
export const userReviewUserFlow: ActionHandler = async (
  context
): Promise<ActionResult> => {
  console.log('  Reviewing user flow document...');

  const userFlowPath = path.join(WEB_APP_PM_DIR, 'user_flow.md');
  const approvalPath = path.join(WEB_APP_PM_DIR, 'user_flow_approval.json');
  
  // Check if user flow document exists
  if (!await fs.pathExists(userFlowPath)) {
    return {
      success: false,
 ...
 * Validate user flow
 */
export const validateUserFlow: ActionHandler = async (
  context
): Promise<ActionResult> => {
  console.log('  Validating user flow logic and assets...');

  const filesToCheck = [
    { name: 'vision_details.json', path: path.join(WEB_APP_PM_DIR, 'vision_details.json') },
    { name: 'vision_approval.json', path: path.join(WEB_APP_PM_DIR, 'vision_approval.json') },
    { name: 'user_flow.md', path: path.join(WEB_APP_PM_DIR, 'user_flow.md') },
    { name: 'user_flow_details.json', path: path.join(WEB_APP_PM_DIR, 'user_flow_details.json') },
    { name: 'user_flow_approval.json', path: path.join(WEB_APP_PM_DIR, 'user_flow_approval.json') }
  ];
  ...
  // Check approvals
  const visionApproval = await fs.readJson(filesToCheck[1].path);
  const flowApproval = await fs.readJson(filesToCheck[4].path);
 ...
 */
export const generateRequirements: ActionHandler = async (
  context
): Promise<ActionResult> => {
 ...
 */
export const userReviewRequirements: ActionHandler = async (
  context
): Promise<ActionResult> => {
  console.log('  Reviewing requirements package...');

  const approvalPath = path.join(WEB_APP_PM_DIR, 'requirements_approval.json');
 ...
 */
export const generateDesignBible: ActionHandler = async (
  context
): Promise<ActionResult> => {
  console.log('  Generating Design Bible options...');

  const optionsPath = path.join(WEB_APP_DESIGN_DIR, 'design_bible_options.json');
  const draftPath = path.join(WEB_APP_DESIGN_DIR, 'design_bible_draft.json');
  const previewDir = path.join(WEB_APP_DESIGN_DIR, 'previews');
  const prdPath = path.join(WEB_APP_PM_DIR, 'PRD.md');
  ...
  await fs.ensureDir(WEB_APP_DESIGN_DIR);
...
Once saved, run 'npm run start -- bob' and I will execute the reasoning engine for each.`
  ...
  try {
    ...
    const flavors = draft.directions.map((d: any) => ({
      name: d.name,
      prompt: d.modifiers
    }));
    ...
    await fs.writeJson(optionsPath, { directions }, { spaces: 2 });
 ...
 */
export const approveDesignBible: ActionHandler = async (
  context
): Promise<ActionResult> => {
  console.log('  Approving Design Bible selection...');

  const optionsPath = path.join(WEB_APP_DESIGN_DIR, 'design_bible_options.json');
  const choicePath = path.join(WEB_APP_DESIGN_DIR, 'design_bible_choice.json');
  const styleGuideMd = path.join(WEB_APP_DESIGN_DIR, 'style_guide.md');
  const styleGuideJson = path.join(WEB_APP_DESIGN_DIR, 'style_guide.json');

  if (await fs.pathExists(choicePath)) {
    const choice = await fs.readJson(choicePath);
    if (choice.selected_id) {
      // Ensure style guide files are generated if choice exists but files don't
      if (!await fs.pathExists(styleGuideMd) || !await fs.pathExists(styleGuideJson)) {
...
Present these to the user along with the HTML previews in the '${path.join(WEB_APP_DESIGN_DIR, 'previews')}' folder.
